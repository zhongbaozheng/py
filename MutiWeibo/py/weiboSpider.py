...
from py.dbHelper import WeiboHelper
    ...
    def write_txt(self):
        try:
            ...
            for i in range(1, self.weibo_num2 + 1):
                ...
                print("......正在插入数据......")
                item = WeiboItem(self.username,str(self.filter),self.weibo_num,str(self.followers),str(self.following),
                                 str(self.weibo_content[i - 1]),str(self.publish_time[i - 1]),str(self.up_num[i - 1]),
                                 str(self.retweet_num[i - 1]),
                                 str(self.comment_num[i - 1]))
                weiboHelper = WeiboHelper()
                weiboHelper.insert_item(item)
                print("......插入数据成功......")
